...
(async function()
{
	...
	Menus.prototype.init = function()
	{
		...
		{
			this.addMenuItems(menu, ['new', 'newProject','newDesign', 'open'], parent);
	...
	EditorUi.prototype.init = async function()
	{
			...
			editorUi.showDialog(new PluginsDialog(editorUi, async function(callback)
			{
				...
				if (await requestSync('isPluginsEnabled'))
				{
					var extPluginsBtn = mxUtils.button(mxResources.get('selectFile') + '...', async function()
					{
						...
						alert('Only plugins that do not require eval() can be loaded. See FAQ for details.');
						var paths = await requestSync({
							action: 'showOpenDialog',
							defaultPath: lastDir || (await requestSync('getDocumentsFolder')),
							filters: [
        ...
        (function attachPerPageSaveOverrides() {
            const ui = this; // 在 EditorUi.prototype.init 里

            // 让 File > Save 只调用你的逐页保存
            const actSave = ui.actions.get('save');
            ui.actions.addAction('save', async function () {
                try {
                    // 你自己的入口：逐页保存到 .dftarch
                    await window.DftSaveProjectIndividually(ui);
                    ui.editor?.setStatus?.('Saved all pages (.dftarch).');
                } catch (e) {
                    ui.handleError?.(e, true);
                }
            }, null, null, Editor.ctrlKey + '+S');

            // 让 File > Save As 只换根目录后再逐页保存
            const actSaveAs = ui.actions.get('saveAs');
            ui.actions.addAction('saveAs', async function () {
                try {
                    // 这里如果你需要“强制重新选根目录”，用你自己的选择器即可
                    if (typeof window.DftPickRootDir === 'function') {
                        await window.DftPickRootDir(ui, /*forcePick=*/true);
                    }
                    await window.DftSaveProjectIndividually(ui);
                    ui.editor?.setStatus?.('Saved all pages (.dftarch).');
                } catch (e) {
            ...
            // 保险起见，再钩一层：让 App/LocalFile 的底层保存直接短路
            // const Noop = async () => { }; // 不做任何磁盘写
            // App.prototype.saveFile = Noop;
            // LocalFile.prototype.save = function (_rev, success) { success && success(); };
            // LocalFile.prototype.saveFile = function (_r, success) { success && success(); };
            // LocalFile.prototype.saveAs = function (_t, success) { success && success(); };
        }).call(this);
	...
	App.prototype.chooseFileEntry = async function(fn)
	{
		...
		var paths = await requestSync({
			...
			defaultPath: lastDir || (await requestSync('getDocumentsFolder')),
			filters: [
                { name: 'draw.io Diagrams', extensions: ['drawio', 'xml', 'png', 'dftart', 'svg', 'html'] },
	...
	EditorUi.prototype.normalizeFilename = function(title, defaultExtension)
	{
        ...
        title = title.replace(/\.drawio$/i, '')
        if (!/\.dftart$/i.test(title)) {
            // title += ".dftart";
        }
        // title = "ddddd";
		var tokens = title.split('.');
		var ext = (tokens.length > 1) ? tokens[tokens.length - 1] : '';
		defaultExtension = (defaultExtension != null) ? defaultExtension : 'dftart';
        // alert("defaultextension: " + defaultExtension);
        // defaultExtension = "dftart"; //defaultExtension forced to dftart for desktop
		if (tokens.length == 1 || mxUtils.indexOf(['xml',
            'html', 'drawio', 'png', 'dftart', 'svg' ], ext) < 0)
		{
...
{
            // ---- dftart 项目文件拦截 ----
            if (handleDftartProject.call(this, path, data)) return;  // 拦截成功就不再往下走，打开项目树
            // ---- dftart 项目文件拦截结束 ----
	...
	*/
			...
			try
			{
				if (this.fileObject == null)
				{
					...
					var path = await requestSync({
						...
						filters: this.ui.createFileSystemFilters(ext)  //筛选文件后缀
					});
                    // alert(path);
					if (path != null)
					{
	...
	App.prototype.createFileSystemFilters = function(defaultExt)
	{
        ...
        {
            // alert(this.editor.diagramFileTypes[i].extension);
		...
		// // dftart extension is always available
        // if (defaultExt != 'dftart')
        // {
        //     ext.splice(0, 0, {name: mxResources.get('drawioFile') + ' (.dftart)', extensions: ['dftart']});
        // }
        console.log(ext);
        // alert("dddddsd s: "+ext);
		return ext;
    ...
    // {
    //     if (format !== 'pdf') {
    //         editorUi.handleError({ message: 'Only PDF export is allowed in this version.' });
    //         return;
    //     }
    ...
    // ======= Override File > Save / Save As to "save per page"  =======
    (function () {
        // 让 Save/Save As 选择或复用“项目根目录”
        async function pickRootDir(ui, forcePick) {
            if (!forcePick && ui._projectRootPath) return ui._projectRootPath;
            ...
            if (!picked || !picked.length) throw new Error('已取消');
            ui._projectRootPath = picked[0];  // 记住项目路径
            return ui._projectRootPath;
        }

        // 1) 接管 App 级保存（菜单/快捷键最终会调到这里）
        const _app_saveFile_orig = App.prototype.saveFile;
        App.prototype.saveFile = async function (forceDialog) {
            const ui = this;
            try {
                await pickRootDir(ui, !!forceDialog || !ui._projectRootPath);
                await window.DftSaveProjectIndividually(ui);  // ← 逐页保存
                ui.editor?.setStatus && ui.editor.setStatus('Saved all pages individually.');
            } catch (e) {
        ...
        // 2) 接管 LocalFile.saveFile（有些路径直接调这个）
        const _local_saveFile_orig = LocalFile.prototype.saveFile;
        LocalFile.prototype.saveFile = async function (revision, success, error/*, unloading, overwrite*/) {
            try {
                await pickRootDir(this.ui, false);
                await window.DftSaveProjectIndividually(this.ui);
                success && success();
            } catch (e) {
                error && error(e);
            }
        ...
        // 3) 接管 LocalFile.saveAs（确保“Save As”会弹选择目录）
        const _local_saveAs_orig = LocalFile.prototype.saveAs;
        LocalFile.prototype.saveAs = async function (/*title,*/ success, error) {
            try {
                await pickRootDir(this.ui, true);   // 强制重新选择根目录
                await window.DftSaveProjectIndividually(this.ui);
                success && success();
            } catch (e) {
                error && error(e);
