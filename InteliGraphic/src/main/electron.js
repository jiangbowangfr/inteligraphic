...
// const vm = require('vm');
// 
// const code = fs.readFileSync(path.join(__dirname, 'convert-mmd.js'), 'utf-8');
// vm.runInThisContext(code); // 直接执行这个 js，让它在全局定义函数
// 
// 或者如果你是 module.exports 形式
// const { convertXmlToMermaid } = require('./convert-mmd.js');
// global.convertXmlToMermaid = convertXmlToMermaid;
...
const tclSessions = new Map();

function resolveTclExecutable()
{
    const defaultName = isWin ? 'tclsh.exe' : 'tclsh9.0';
	const candidates = [
		path.join(process.resourcesPath || '', 'tcl', defaultName),
		path.join(process.resourcesPath || '', 'tcl', 'bin', defaultName),
		path.join(process.cwd(), 'vendor', 'tcl', defaultName),
		path.join(process.cwd(), 'vendor', 'tcl', 'bin', defaultName)
	];
...
function buildTclEnv()
{
	const env = Object.assign({}, process.env);

	const libCandidates = [
		path.join(process.resourcesPath || '', 'tcl', 'lib', 'tcl8.6'),
		path.join(process.resourcesPath || '', 'tcl', 'lib', 'tcl8.7'),
		path.join(process.cwd(), 'vendor', 'tcl', 'lib', 'tcl8.6'),
		path.join(process.cwd(), 'vendor', 'tcl', 'lib', 'tcl8.7')
	];

	for (let i = 0; i < libCandidates.length; i++)
	{
		if (fs.existsSync(libCandidates[i]))
		{
			env.TCL_LIBRARY = libCandidates[i];
			break;
		}
	}
...
class TclSession
{
	...
	start()
	{
		...
		const exe = resolveTclExecutable();
		...
		this.proc.on('exit', (code, signal) =>
		{
			if (this.pending)
			{
				...
				this.pending = null;
				clearTimeout(pending.timer);
				pending.reject(new Error('tclsh exited unexpectedly (code=' + code + ', signal=' + signal + ')'));
			}
			this.proc = null;
		});

		this.writeRaw('fconfigure stdout -buffering line\n');
		this.writeRaw('fconfigure stderr -buffering line\n');
	}
	...
	writeRaw(script)
	{
		...
		{
			throw new Error('Tcl session is not available');
		}

		this.proc.stdin.write(script, 'utf8');
	}
	...
	handleStdout(chunk)
	{
		...
		const marker = '__TCL_DONE__' + this.pending.token + '__RC=';
		const idx = this.pending.buffer.indexOf(marker);

		if (idx === -1) return;

		const rest = this.pending.buffer.slice(idx + marker.length);
		const match = rest.match(/^(\d+)\r?\n?/);

		if (!match) return;

		const rc = Number(match[1]);
		const visible = this.pending.buffer.slice(0, idx).replace(/\r?\n$/, '');
		const stderrText = this.pending.stderr.replace(/\r?\n$/, '');
		const stdoutText = rc === 0 ? visible : '';
		const mergedError = rc === 0 ? stderrText : [visible, stderrText].filter(Boolean).join('\n');
	...
	{
		this.queue = this.queue.then(() => this.execNow(cmd, timeoutMs));
		return this.queue;
	}

	execNow(cmd, timeoutMs)
	{
		this.start();
		...
		return new Promise((resolve, reject) =>
		{
			...
			const payload = Buffer.from(String(cmd == null ? '' : cmd), 'utf8').toString('base64');
			...
			const timer = setTimeout(() =>
			{
				if (this.pending && this.pending.token === token)
				{
					this.pending = null;
					this.killProcess();
					reject(new Error('Tcl command timeout after ' + timeoutMs + 'ms'));
				}
			}, timeoutMs);
			...
			const script = [
				'set __dft_cmd [binary decode base64 {' + payload + '}]',
				'set __dft_rc [catch {uplevel #0 $__dft_cmd} __dft_res __dft_opts]',
				'if {$__dft_res ne ""} { puts $__dft_res }',
				'puts "__TCL_DONE__' + token + '__RC=$__dft_rc"'
			].join('\n') + '\n';

			try
			{
				this.writeRaw(script);
			}
			catch (e)
			{
				this.pending = null;
	...
	dispose()
	{
		if (this.pending)
		{
			...
			this.pending = null;
			clearTimeout(pending.timer);
			pending.reject(new Error('Tcl session terminated'));
		}
		this.killProcess();
	}
}

function getTclSession(ownerId)
{
	let session = tclSessions.get(ownerId);

	if (!session)
	{
		session = new TclSession(ownerId);
		tclSessions.set(ownerId, session);
	}

	return session;
}

function closeTclSession(ownerId)
{
	const session = tclSessions.get(ownerId);

	if (session)
	{
		tclSessions.delete(ownerId);
		session.dispose();
	}
}

function resolveSlangExecutable(explicitPath)
{
	const explicit = explicitPath != null ? String(explicitPath).trim() : '';
	...
	const defaultName = isWin ? 'slang.exe' : 'slang';
	const platformDir = isWin ? 'win-x64' : (isMac ? 'mac' : 'linux-x64');
	const candidates = [
		path.join(process.resourcesPath || '', 'slang', defaultName),
		path.join(process.resourcesPath || '', 'tools', 'slang', defaultName),
		path.join(process.resourcesPath || '', 'tools', 'slang', platformDir, defaultName),
		path.join(process.cwd(), 'vendor', 'slang', defaultName),
		path.join(process.cwd(), 'vendor', 'slang', 'bin', defaultName),
		path.join(process.cwd(), 'tools', 'slang', defaultName)
	];
...
function resolveHdlBridgeScript(explicitPath)
{
	const explicit = explicitPath != null ? String(explicitPath).trim() : '';
	...
	if (explicit)
	{
		...
		throw new Error('HDL bridge script not found: ' + explicit);
	}

	const candidates = [
		path.join(__dirname, 'dft_slang_bridge.cjs'),
		path.join(process.cwd(), 'dft_slang_bridge.cjs'),
		path.join(process.resourcesPath || '', 'dft_slang_bridge.cjs'),
		path.join(process.resourcesPath || '', 'app.asar.unpacked', 'src', 'main', 'dft_slang_bridge.cjs')
	];
	...
	throw new Error('dft_slang_bridge.cjs not found. Put it next to electron.js or pass payload.bridgePath.');
}

function runHdlParse(payload, timeoutMs)
{
	payload = payload || {};
	...
	{
		let settled = false;
		const bridgeScript = resolveHdlBridgeScript(payload.bridgePath);
		const resolvedPayload = Object.assign({}, payload, {
			slangPath: resolveSlangExecutable(payload.slangPath)
		});
		...
		const timer = setTimeout(() =>
		{
			...
			reject(new Error('HDL parse timeout after ' + timeoutMs + 'ms'));
		}, timeoutMs);
		...
		child.on('close', (code) =>
		{
			...
			if (code !== 0 && !stdout)
			{
				finish(new Error(stderr || ('HDL bridge exited with code ' + code)));
				return;
			}
			...
			{
				finish(new Error('Invalid HDL bridge JSON: ' + e.message + (stderr ? ('\n' + stderr) : '')));
				return;
			}
			...
			parsed.slangPath = resolvedPayload.slangPath;
			parsed.bridgePath = bridgeScript;
			finish(null, parsed);
		});

		try
		{
			child.stdin.write(JSON.stringify(resolvedPayload || {}), 'utf8');
			child.stdin.end();
		}
		catch (e)
		{
...
function createWindow (opt = {})
{
	...
	const tclOwnerId = mainWindow.webContents.id;
	...
	if (body != null)
	{
		...
		if (typeof body === 'string')
		{
			...
			else
			{
                ...
                // console.log("headBinay:", headBinay);
			}
		}
		else
		{
        ...
        if(c1 == 'g' && c2 == 'r' && c3 == 'a' && c4 == 'p' && c5 == 'h')
        {
            return true;
        }

        if(c1 =='#' && c2 =='y' && c3 == 'a' && c4=='m' && c5=='l')
        {
            return true;
        }

        if (c1 == '#' && c2 == 'd' && c3 == 'f' && c4 == 't' && c5 == 's' && c6 == 'p' && c7 == 'e' && c8 == 'c') {
            return true;
        }


        //flow
        if(c1=='f' && c2 == 'l' && c3=='o' && c4=='w'){
            return true;
        }
...
//原来是校验内容格式 禁止写到 app 安装目录 现在变成不再校验内容格式仍然禁止写到 app 安装目录
async function writeFile(filePath, data, enc) {
    if (path.resolve(filePath).startsWith(appBaseDir)) {
...
async function copyFilePath(srcFile, dstFile)
{
	if (!srcFile || !dstFile)
	{
		throw new Error('Invalid file path');
	...
	if (path.resolve(srcFile).startsWith(appBaseDir) || path.resolve(dstFile).startsWith(appBaseDir))
	{
		throw new Error('Invalid file path');
	}

	await fsProm.mkdir(path.dirname(dstFile), { recursive: true });
	await fsProm.copyFile(srcFile, dstFile);
	return true;
}
...
async function deleteDir(dir)
{
	...
	if (resolved.startsWith(appBaseDir))
	{
		throw new Error('Refusing to delete application directory');
	}

	await fsProm.rm(resolved, { recursive: true, force: true });
}
...
ipcMain.on("rendererReq", async (event, args) => 
{
	...
	try
	{
        ...
        {
        case 'ensureDirs':
            await fsProm.mkdir(args.path, { recursive: true });
            ret = true;
            break;
		case 'saveFile':
		...
		case 'writeFile':
			...
			break;
		case 'copyFile':
			ret = await copyFilePath(args.src, args.dst);
			break;
		case 'saveDraft':
		...
		case 'deleteFile':
			...
			break;
		case 'deleteDir':
			ret = await deleteDir(args.path || args.file);
			break;
		case 'fileStat':
		...
		case 'unwatchFile':	
			...
			break;
		case 'tclExec':
			ret = await getTclSession(event.sender.id).exec(args.cmd, args.timeoutMs || 30000);
			break;
		case 'tclReset':
			ret = await getTclSession(event.sender.id).reset();
			break;
		case 'tclStop':
			closeTclSession(event.sender.id);
			ret = true;
			break;
		case 'hdlParse':
			ret = await runHdlParse(args.payload || {}, (args.payload && args.payload.timeoutMs) || 45000);
			break;
		case 'exit':
			app.quit();
			break;
