...
// boost/regex.hpp removed: replaced with <regex> (already included above)
#include "dbChip.h"
#include "dbCommon.h"
                                           ...
                                           std::ifstream& report)
{
  ...
  while (!report.eof()) {
    ...
    std::getline(report, line);
    std::string sources;
    std::smatch sources_match;
    if (std::regex_match(line, sources_match, srcs)) {
