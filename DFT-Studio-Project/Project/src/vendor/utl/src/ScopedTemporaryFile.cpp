...
#else
  if (filename_.size() >= 3
      && filename_.compare(filename_.size() - 3, 3, ".gz") == 0) {
    throw std::runtime_error(
        "gzip streams not supported in standalone build: " + filename_);
  }
#endif
}
...
#else
  stream_ = nullptr;  // always nullptr in no-gzip mode
#endif
